...
class N2OEmissions:
    ...
    def calculate_for(self, farmed_crop: FarmedCrop):
        instance = N2OEmissions(self._references)
        ...
        self.calculate_synthetic(farmed_crop.activities.fertilizing.synthetic.n, instance.total)
        self.calculate_organic(farmed_crop.activities.fertilizing.organic.n, instance.total)
        self.calculate_excreta(farmed_crop.activities.fertilizing.excreta.n, instance.total)
    ...
    def calculate_total(self, fertilizers: Fertilizers, soil: Soil, crop: Crop, weather: Weather):
        climate_n2o_n = self._references.get_climate_zones()
        ...
        self.total = np.amin(
                            ...
                            np.exp(
                                ...
                                climate_n2o_n[weather.eco_clim_zone.value] + crop_n2o_n[crop.characteristics.crop_name]) -
                            np.exp(
                                0.475 +
                                (0 if soil.org_carbon < 0.01 else 0.526 if soil.org_carbon <= 0.03 else 0.6334) +
                                (0 if soil.phH20 < 5.5 else -0.4836 if soil.phH20 > 7.3 else -0.0693) +
